...
$(document).ready( function () {
	...
	$('.typeahead').bind('typeahead:select', function(ev, suggestion) {
	    ...
	    INFOCOMP.datosAcliente(suggestion);
	    $('#inputCliPorCod').val('');
	});
		    ...
		    $.ajax( { url : _pathCodcliente,
			          ...
			          success: function(data, textStatus, jqXHR) 
			          {
						...
						if (dataObj.status == 'ok') {
							...
							INFOCOMP.datosAcliente(dataObj);
							...
							$('#inputCliPorCod').val('');
							INFOCOMP.alerta('Código no encontrado !!', $('#inputBuscarCli'));
						}
		        	  },
		        	  error: function(jqXHR, textStatus, errorThrown) 
		        	  {
		            	...
		            	INFOCOMP.alerta('Error al buscar código !!', $('#inputBuscarCli'));
		        	}
		    } );
		}
	...
	$('#btnGenList').click(function (event) {
		...
		paramString += '&idcli=' + INFOCOMP.clienteSel.id;
