...
$(document).ready( function () {
    ...
    // Foco inicial en input fecha desde
	$('#fechaDesde').focus();
...
//console.log('Select cliente: ' + suggestion.id + ' - ' + suggestion.cliente);
	    ...
	    RESCTA.datosAcliente(suggestion);
	    $('#inputCliPorCod').val('');
	});
...
//console.log('Cliente: ' + dataObj.cliente + ' - Id dom: ' + dataObj.iddom);
							...
							$('#inputCliPorCod').val('');
							RESCTA.alerta('Código no encontrado !!', $('#inputBuscarCli'));
						}
		        	  },
		        	  error: function(jqXHR, textStatus, errorThrown) 
		        	  {
		            	...
		            	RESCTA.alerta('Error al buscar código !!', $('#inputBuscarCli'));
		        	}
		    } );
		} else RESCTA.alerta('Ingrese código para buscar !!', $('#inputCliPorCod'));

	});

	// Botón 'Limpiar seleccion'
	...
	$('#btnGenList').click(function (event) {
        ...
        if ( inputClie.trim() !== '') {
			...
			paramString += '&idcli=' + RESCTA.clienteSel.id;
			//paramString += '&iddom=' + RESCTA.clienteSel.iddom;

//console.log('Params: ' + paramString);
			// Envio de datos a controller por get...
			window.open(_pathResumen + paramString, '_blank');

        } else RESCTA.alerta('Seleccione un cliente !!', $('#inputBuscarCli'));
